...
#Histogram of length at capture 
...
ndata <- with(dat_q3, data_frame(Length_at_capture = seq(min(Length_at_capture), max(Length_at_capture),
                                                length = 58)))

ndata <- add_column(ndata, fit = predict(glm, newdata = ndata, type = 'response'))

plt <- ggplot(ndata, aes(x = Length_at_capture, y = fit)) +
  geom_line() +
  xlab('Length at capture')+
  ylab('Odds of transition')
...
ndata <- bind_cols(ndata, setNames(as_tibble(predict(glm, ndata, se.fit = TRUE)[1:2]),
                                   c('fit_link','se_link')))
ndata <- mutate(ndata,
...
q1.5 = ndata %>% 
  select(Length_at_capture, right_upr, right_lwr) %>%
  filter(Length_at_capture > 299 & Length_at_capture < 301)
q1.5

ndata

#Lower 95% CI for length = 300mm - 0.130
#Upper 95% CI for length = 300mm - 0.358


#Question 1.6 
#These data do not quite meet the assumptions of linearity between the independent and dependent variable 
#There was a right skew to the data as shown in the histogram above
#There could be a much more complex relationship between length and probability of sex transition